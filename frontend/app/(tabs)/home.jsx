...
const Home = () => {
  ...
  const metricCards = [
    ...
    },
    {
      ...
      bodyText: "Effortlessly search and find articles that inspire and inform.",
      category: "Article",
    },
    {
      route: "/favourite",
  ...
  useEffect(() => {
    const newFilteredCards = metricCards.filter(({ title, bodyText }) =>
        ...
        <View className="bg-optimistic-gray-10 p-4 rounded-lg mb-4">
          <Link href="/map" asChild>
            <Pressable className="relative w-full rounded-xl mb-4" style={{ aspectRatio: 1.5 }}>
              <Image source={require('../../assets/start.svg')} className="w-full h-full rounded-xl" />
          ...
          {['Journal', 'Article', 'Landmarks', 'Achievement'].map((section) => {
            ...
            if (sectionCards.length === 0) {
              return null // Skip rendering this section if no cards are present
            }
            ...
            return (
              <View key={section}>
                ...
                {/* Render Cards for the Current Section */}
