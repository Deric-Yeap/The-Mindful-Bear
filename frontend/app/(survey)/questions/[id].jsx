...
const QuestionPage = () => {
  ...
  const LikertEmoticons = {
    ...
    3: 'emoticon-happy-outline',
    4: 'emoticon-excited-outline',
  }
  ...
  return (
    <ScrollView className="flex-1 p-4 bg-mindful-brown-10 mt-4">
          ...
          currentQuestion.optionSet.options.length > 0 ? (
            currentQuestion.optionSet.options.map((option) => (
              <Pressable
                key={option.id}
                onPress={() =>
                  handleAnswerChange(currentQuestion.questionID, option.value)
                }
                className={`flex flex-row justify-between items-center p-4 mb-2 rounded-xl bg-white ${
                  answers[currentQuestion.questionID] === option.value
                    ? 'bg-serenity-green-50'
                    : ''
                }`}
              >
                <MaterialCommunityIcons
                  name={LikertEmoticons[option.value]}
                  size={30}
                  color={
                    answers[currentQuestion.questionID] === option.value
                      ? 'white'
      ...
      <Pressable
        ...
        className={`py-4 px-6 rounded-full mt-8 ${
          isOptionSelected ? 'bg-mindful-brown-70' : 'bg-optimistic-gray-40'
        }`}
