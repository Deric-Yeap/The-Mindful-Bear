...
const Questionaire = () => {
  const { sessionStarted, formData, sessionID, completedForms: initialCompletedForms, start } = useLocalSearchParams(); 
  ...
  const handleStart = () => {
    ...
    const uncompletedCompulsoryForms = forms.filter(form => form.is_compulsory && !completedForms.includes(form.id));
      ...
      <View className="flex-1 p-6 bg-optimistic-gray-10">
        ...
        <View className="space-y-4">
          {forms.map((form, index) => {
            ...
            const isUncompletedCompulsory = isCompulsory && !isCompleted && startPressed; 
        ...
        {showError && (
          <Text className="text-red-500 text-center mt-4">Please complete all required forms</Text>
        )}
