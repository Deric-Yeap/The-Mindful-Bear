  ...
  const handleDelete = async (id) => {
    try {
      ...
      setLandmarks((prevLandmarks) =>
        prevLandmarks.filter((landmark) => landmark.landmark_id !== id)
      )
      ...
      <ScrollView className="px-4 mt-4">
        ...
        <View className="flex-wrap flex-row justify-between">
          {landmarks.map((landmark, index) => (
            <View
              key={index}
              className="bg-mindful-brown-20 rounded-2xl w-[48%] mb-4"
            >
              <Image
              ...
              <View className="p-3 rounded-b-2xl bg-mindful-brown-60">
                ...
                <View className="flex-row justify-between mt-2">
                  <Link
                    href={`/landmarkCreator?landmark=${encodeURIComponent(JSON.stringify(landmark))}`}
                    asChild
                  >
                    <TouchableOpacity className="bg-mindful-brown-50 px-3 py-1 rounded-full">
                      <Text className="text-mindful-brown-100 font-urbanist-bold">
                        Modify
                      </Text>
                    </TouchableOpacity>
                  </Link>
