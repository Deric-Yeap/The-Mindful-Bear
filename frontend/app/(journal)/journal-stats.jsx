...
const JournalStats = ({ title = "Journal Stats", subtitle = "Please Select a date range" }) => {
    ...
    const handleDayPress = (day) => {
        ...
        if (!startDate || (startDate && endDate)) {
            ...
            // If the new date is before the start date, reset the range
            setStartDate(selectedDate);
            setEndDate(null);
        } else {
            // Set the end date
        ...
        // Remove this line to keep the modal open
    ...
    const getMarkedDates = () => {
        ...
        if (startDate && endDate) {
            ...
            marked[startKey] = {
                selected: true,
                startingDay: true,
                color: '#005700', // Dark green for starting day
                textColor: 'white',
                // Use a custom style to achieve half-circle effect
                customStyles: {
                    container: {
            ...
            marked[endKey] = {
                selected: true,
                endingDay: true,
                color: '#005700', // Dark green for ending day
                textColor: 'white',
                // Use a custom style to achieve half-circle effect
                customStyles: {
                    container: {
            ...
            // Mark the dates in between with a lighter blue
            let currentDate = new Date(startDate);
            while (currentDate <= endDate) {
                marked[currentDate.toISOString().split('T')[0]] = {
                    color: '#add8e6', // Light blue for the range
                };
                currentDate.setDate(currentDate.getDate() + 1);
            }
        } else if (startDate) {
            const startKey = startDate.toISOString().split('T')[0];
            marked[startKey] = { selected: true, color: '#005700', textColor: 'white' }; // Dark green for selected date
        }
        return marked;
    };
    ...
    return (
        <SafeAreaView className="bg-optimistic-gray-10 h-full p-4 space-y-4">
            ...
            <View className="flex-row items-center justify-between">
                ...
                >
                    <MaterialCommunityIcons name="calendar-month-outline" size={30} color="white" />
                </TouchableOpacity>
            </View>
                ...
                <View className="flex-1 justify-center items-center bg-black bg-opacity-50">
                    <View className="bg-white rounded-lg w-80 p-4 shadow-lg">
                        <Calendar
                            ...
                            current={today.toISOString().split('T')[0]} // Current date
            ...
            <View className="pb-20">
                <BarChart
                    ...
                    yAxisLabelTexts={Array.from({ length: noOfSections + 1 }, (_, i) => (i * stepValue).toString()).concat(maxValue.toString())}
