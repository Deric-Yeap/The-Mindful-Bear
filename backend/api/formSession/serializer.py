...
class ScoreAggregationSerializer(serializers.Serializer):
    ...
    def categorize_scores_by_level(self, session_ids):
            """
            ...
            sessions = FormSession.objects.filter(SessionID__in=session_ids)
            ...
            # Get PSS 'before' and 'after' scores
            pss_before_scores = sessions.filter(FormID=3, id__in=self.get_before_ids(sessions, FormID=3))
            pss_after_scores = sessions.filter(FormID=3, id__in=self.get_after_ids(sessions, FormID=3))

            # Get SMS 'before' and 'after' scores
            sms_before_scores = sessions.filter(FormID=5, id__in=self.get_before_ids(sessions, FormID=5))
            sms_after_scores = sessions.filter(FormID=5, id__in=self.get_after_ids(sessions, FormID=5))

            # Categorize PSS and SMS scores into low, moderate, and high groups
            pss_before_categories = categorize_score([float(score.aggregatedScore) for score in pss_before_scores], pss_thresholds)
            pss_after_categories = categorize_score([float(score.aggregatedScore) for score in pss_after_scores], pss_thresholds)
            sms_before_categories = categorize_score([float(score.aggregatedScore) for score in sms_before_scores], sms_thresholds)
            sms_after_categories = categorize_score([float(score.aggregatedScore) for score in sms_after_scores], sms_thresholds)
    ...
    def get_before_ids(self, sessions, FormID):
        """Helper method to get the minimum IDs for 'before' scores based on FormID."""
        return sessions.filter(FormID=FormID).values('SessionID').annotate(before_score=Min('id')).values_list('before_score', flat=True)

    def get_after_ids(self, sessions, FormID):
        """Helper method to get the maximum IDs for 'after' scores based on FormID."""
        return sessions.filter(FormID=FormID).values('SessionID').annotate(after_score=Max('id')).values_list('after_score', flat=True)